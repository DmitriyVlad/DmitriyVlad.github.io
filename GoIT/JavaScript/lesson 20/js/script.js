...
console.log('1. Sorted by skills: ', skills);

// 2. Sorted by names
var names = _.map(arr, function(o) {
  return { "name": o.name, "friends": o.friends}
});

names = _.map(_.sortBy(names, 'friends.length'), 'name');

console.log('2. Sorted by names: ', names);

// 3. Sorted by friends
var allFriends = _.map(arr, 'friends');
allFriends = _.flatten(allFriends);
allFriends = _.map(allFriends, 'name');
allFriends = _.union(allFriends);
allFriends = _.sortBy(allFriends);

console.log('3. Sorted by friends: ', allFriends);
